        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 md:gap-12 lg:gap-16 items-center">
          ...
          <AnimateIn direction="up">
            <div className="relative w-full aspect-square max-w-[260px] sm:max-w-[300px] md:max-w-[360px] lg:max-w-[420px] mx-auto">
              ...
              {/* Dashed orbit ring — the path the circles travel on */}
              <motion.div
              ...
              {/*
                Each image orbits by:
                1. Rotating the arm (full 360°) — this moves the image around the ring
                2. Counter-rotating the image itself — keeps it upright
                Images are placed at the TOP of the arm (top: 0, centered horizontally)
                which puts them exactly on the ring edge
              */}
              {orbitImages.map((img, i) => {
                const startDeg = i * 120; // 0°, 120°, 240° — evenly spaced
                return (
                  <motion.div
                    key={i}
                  ...
                  >
                    {/* Image sits at the top edge of the arm = on the ring */}
                    <motion.div
                      className="absolute left-1/2 top-0 -translate-x-1/2 -translate-y-1/2 w-[27%] aspect-square rounded-full overflow-hidden border-2 border-white shadow-lg"
                      animate={{ rotate: [-(startDeg), -(startDeg + 360)] }}
                      transition={{ duration: 18, repeat: Infinity, ease: "linear" }}
                    >
              ...
              {/* Center badge — static, always in the middle */}
