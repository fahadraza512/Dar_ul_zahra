        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {programs.map((p, i) => (
            <AnimateIn key={i} direction="up" delay={i * 0.12}>
              ...
              >
                <motion.div
                  className="w-14 h-14 bg-orange-50 rounded-xl flex items-center justify-center mb-8 text-primary"
                  whileHover={{ rotate: 8, scale: 1.1 }}
                  transition={{ type: "spring", stiffness: 300 }}
                >
                  {p.icon}
                </motion.div>
                <h3 className="text-xl font-bold mb-4 font-manrope text-[#0c1525]">{p.title}</h3>
                <p className="text-[#5e6d82] mb-10 leading-relaxed flex-1">{p.desc}</p>
                <div className="pt-6 border-t border-gray-100 flex justify-between items-center">
                  <span className="text-[10px] font-black text-gray-400 tracking-widest uppercase">{p.deadline}</span>
                  <motion.a href="#" className="text-primary font-bold text-sm" whileHover={{ x: 3 }}>
                    Details →
                  </motion.a>
                </div>
              </motion.div>
